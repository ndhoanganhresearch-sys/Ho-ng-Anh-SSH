r"""Create a realistic Blender/MCP LiDAR T0-T5 tunnel dataset.

This generator builds a richer tunnel scene than the formula-only
``time_series_deformation`` dataset:

- curved segmented tunnel lining with rings and joints;
- rails, sleepers, cable trays, lights, walkway, drainage, target spheres;
- progressive T0-T5 deformation patterns;
- multi-station TLS-style raycasting with occlusion;
- epoch-specific scanner pose bias so registration is not identity;
- material-dependent intensity, range/grazing noise, and beam dropout.

Run from ``tunnel_project`` while Blender MCP is listening on localhost:9876:

    ..\.venv\Scripts\python.exe tools\create_blender_lidar_t0t5_realistic.py

The script sends Blender Python code through the MCP socket and writes output to
``data/blender_lidar_t0t5_realistic`` by default.
"""
...
OUT_DIR = ROOT / "data" / "blender_lidar_t0t5_realistic"
...
def convert_txt_epochs_to_las(out_dir: Path) -> None:
    ...
    for txt_path in sorted(out_dir.glob("T[0-5].txt")):
        ...
        las.green = gray
        las.blue = gray
        las_path = txt_path.with_suffix(".las")
        las.write(str(las_path))
        print(f"LAS written: {las_path}")
...
def main() -> int:
    ...
    parser.add_argument("--save-code", action="store_true", help="Only write the generated Blender Python code next to the output directory")
    parser.add_argument("--skip-las", action="store_true", help="Do not convert generated TXT epochs to LAS after Blender finishes")
    ...
    if args.save_code:
        out_dir.mkdir(parents=True, exist_ok=True)
        code_path = out_dir / "create_scene_inside_blender.py"
        code_path.write_text(code, encoding="utf-8")
        print(f"Blender code written to: {code_path}")
        return 0
