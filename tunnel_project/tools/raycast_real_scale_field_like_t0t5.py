...
OUT_DIR = ROOT / 'data' / 'real_scale_railway_tunnel_t0t5' / 'field_like_raycast_dataset'
...
def create_clean_lining(out_dir: Path):
    ...
    manifest['dataset']='field_like_raycast_clean_lining_dataset'; manifest['source_dataset']=str(out_dir); manifest['purpose']='Lining-only subset from field-like TLS raycast for Step 6 settlement measurement'; manifest['epochs']=exports
    (clean/'manifest.json').write_text(json.dumps(manifest, indent=2), encoding='utf-8')
    (clean/'README.md').write_text('# Field-like Raycast Clean Lining T0-T5\n\nLining-only subset. Load T0.las as reference and T1-T5 as monitoring epochs for Step 6.\n', encoding='utf-8')
...
def main():
    ...
    code=BLENDER_CODE.replace('__OUT_DIR__', str(out_dir).replace('\\','\\\\')).replace('__RAY_STEP__', str(args.ray_step)).replace('__LABELS_JSON__', json.dumps(LABELS))
