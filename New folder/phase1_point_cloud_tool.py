"""
Standalone Point Cloud Analysis Tool - LOCAL AI EDITION (Ollama) - v4.4
...
Changelog:
  ...
  v4.4 - Tab Interface UX overhaul
...
# ──────────────────────────────────────────────────────────
# MAIN APP  — TAB INTERFACE  (v4.4)
# ──────────────────────────────────────────────────────────

class AnalysisApp:
    def __init__(self, master):
        self.master = master
        master.title("Point Cloud Analysis Tool  v4.4  |  CBNU Smart Structure Lab")
    ...
    def create_mesh(self):
        def task():
            try:
                self.safe_sts("Creating mesh..."); self.safe_prog(20)
                pcd = o3d.geometry.PointCloud(); res = self.analysis_results
                if "ground" in res:
                    pcd += res["ground"]; pcd += res["wall1"]; pcd += res["wall2"]
                else:
                    pcd += res["main_surface"]; pcd += res["damage_points"]
                pcd = pcd.voxel_down_sample(0.1)
                self.safe_prog(50); pcd.estimate_normals()
                self.safe_prog(80)
                mesh = o3d.geometry.TriangleMesh \
                    .create_from_point_cloud_ball_pivoting(
                        pcd, o3d.utility.DoubleVector([0.1, 0.2]))
                self.safe_sts("✅ Mesh ready."); self.safe_prog(100)
                self._o3d([mesh], "3D Mesh")
            except Exception as e: self.safe_sts(f"Mesh error: {e}")
            finally: self.master.after(1000, lambda: self.safe_prog(0))
        threading.Thread(target=task, daemon=True).start()
